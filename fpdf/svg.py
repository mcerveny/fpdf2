...
def apply_styles(stylable, svg_element):
    ...
    try:
        tforigin = svg_element.attrib["transform-origin"]
    except KeyError:
        pass
    else:
        args = number_split.split(tforigin)
        if len(args) == 2:
            cx = resolve_length(args[0])
            cy = resolve_length(args[1])
        elif len(args) == 1:
            cx = resolve_length(args[0])
            cy = 0
        else:
            raise ValueError(f"unsupported transform-origin {tforigin}")

        if stylable.transform is not None:
            stylable.transform = stylable.transform.about(cx, cy)
